...
initial begin
  ...
  class fifo_generator;
    ...
    task run();
      ...
      for (int i = 0; i < num_writes; i++) begin
		...
		$display("[GEN] WRITE data=%0d", tr.data);
  ...
  class write_driver;
    ...
    task run();
      ...
      forever begin
        gen2wdrv.get(tr);
$display("[DRV] WRITE data=%0d", tr.data);
    ...
    task run();
      ...
      forever begin
        ...
        if (w_valid && w_ready) begin
          mon2sb.put(w_data);
	$display("[iMon] captured WRITE data=%0d", w_data);
    ...
    task run();
      ...
      forever begin
        ...
        if (r_valid && r_ready) begin
          mon2sb.put(r_data);
$display("[oMon] captured READ data=%0d", r_data);
...
//functional coverage 
	int level;     // model FIFO occupancy = exp_q.size()
    int cov_op;    // 0=push, 1=pop
    ...
    coverpoint level {
        ...
        bins low      = {[2:3]};
        bins mid      = {[4:DEPTH/2]};
        bins high     = {[DEPTH/2+1:DEPTH-2]};
        bins nearfull = {DEPTH-1};
        bins full     = {DEPTH};
      }
