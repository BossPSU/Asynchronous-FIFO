...
class fifo_scoreboard extends uvm_scoreboard;
	...
	// ----------------------------------------------------------
    	// flush: discard all state between composite-test scenarios.
    	// Call from fifo_full_seq between scenarios to prevent shadow
    	// queue entries from one scenario bleeding into the next.
    	// Logs a warning if items are discarded.
    	// ----------------------------------------------------------
    	function void flush_shadow(string scenario_label = "");
        	int unsigned leftover = shadow_q.size();
        	if (leftover > 0) begin
            		`uvm_warning("SB_FLUSH",
                		$sformatf("Flushing %0d leftover shadow-queue item(s) before '%s'",
                		          leftover, scenario_label))
            		shadow_q.delete();
        	end else begin
            		`uvm_info("SB_FLUSH",
